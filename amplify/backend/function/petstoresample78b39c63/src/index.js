    ...
    try{
       addResourceEntities(entities, actionMap[event.httpMethod + event.resource], event.pathParameters);
        ...
        if (authResult.decision == 'ALLOW') { //action is allowed by AVP
          ...
          return buildResponse(200, 'Successful Support Requests backend processing response for ' + event.httpMethod + ' ', authResult);
          ...
          return buildResponse(403, "You are not authorized on Support case to perform this action. " + event.httpMethod + ' ', authResult);
        }
    }catch(err){
